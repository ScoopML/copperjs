...
const Square = function () {
  ...
  const diff = function (vdom, container, oldDom) {
    ...
    if (!oldDom) {
      ...
      oldDom._virtualElement = vdom; // Lets create a collection of keyed elements
  ...
  function updateComponent(newVirtualElement, oldComponent, container, domElement) {
    // Lifecycle method
    oldComponent.componentWillReceiveProps(newVirtualElement.props); // Lifecycle method

    if (oldComponent.shouldComponentUpdate(newVirtualElement.props)) {
      const prevProps = oldComponent.props; // Invoke LifeCycle

      oldComponent.componentWillUpdate(newVirtualElement.props, oldComponent.state); // Update component

      oldComponent.updateProps(newVirtualElement.props); // Call Render
      // Generate new vdom

      const nextElement = oldComponent.render();
      nextElement.component = oldComponent; // Recursively diff again

      diff(nextElement, container, domElement, oldComponent); // Invoke LifeCycle
  ...
  const mountSimpleNode = function (vdom, container, oldDomElement, parentComponent) {
    ...
    newDomElement._virtualElement = vdom; // TODO: Remove old nodes
  ...
  function jsToCss(s) {
    ...
    let transformedText = s.replace(/([A-Z])/, '-$1').toLowerCase(); // borderBottom transform to border-bottom
  ...
  class Component {
    ...
    setState(nextState) {
      ...
      let newvdom = this.render(); // Recursively diff
